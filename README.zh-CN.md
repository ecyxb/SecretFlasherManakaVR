...
- VR 光标可能和手部姿态不匹配。通常在游戏内重置为第一人称视角可以恢复。
- 存在未知原因导致卡死、崩溃。遇到这种情况时，可以尝试在 `com.codex.secretflashermanaka.vr.cfg` 里关闭后处理或其他渲染相关配置。
- 镜子和反射效果已关闭，目前暂时没有修复或恢复计划。

## 风险警告

本包没有和其他 Mod 进行适配或测试。项目内部存在大量 Hook 和特殊运行时处理逻辑，因此不保证能和其他 Mod 兼容。

本包尚未进行完整流程测试。

输入映射主要按 Quest 3 / Oculus Touch 手柄测试。其他 SteamVR 手柄可能需要自行配置绑定，功能不保证完整。

在场景切换、特殊演出镜头或 GameOver 镜头中，VR 视角可能短暂黑屏、错位或抖动，通常等待切换结束或重置视角可恢复。

...
- 光标可能和手部姿态不匹配。游戏内重置为第一人称一般可以解决。
- 可能存在未知情况下的游戏卡死或崩溃。遇到这种情况时，可以尝试在 `com.codex.secretflashermanaka.vr.cfg` 里关闭后处理或其他有关渲染的配置。
- 关闭了镜子和反射效果，暂无考虑修复。

## 风险警告

本包没有和其他任何 Mod 进行适配或测试。项目内部存在大量 Hook 和特殊处理函数，因此不保证能和其他 Mod 兼容。

本包尚未进行完整流程测试。

输入映射主要按 Quest 3 / Oculus Touch 手柄测试。其他 SteamVR 手柄可能需要自行配置绑定，功能不保证完整。

场景切换、特殊演出镜头或 GameOver 镜头中，VR 视角可能短暂黑屏、错位或抖动，通常等待切换结束或重置视角可恢复。
